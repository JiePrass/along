...
export default function Header() {
    ...
    // LOGIKA 1: Lock Scroll saat menu buka
    useEffect(() => {
        if (menuOpen) {
    ...
    // LOGIKA 2: Scroll Header Show/Hide
    useEffect(() => {
        const handleScroll = () => {
            const currentScrollY = window.scrollY;

            // Warna bg putih jika scroll > 50
            setIsScrolled(currentScrollY > 50);

            // Muncul/Sembunyi ala YouTube
        ...
        <>
            ...
            >
                <div className="container mx-auto px-6 md:px-12 lg:px-16 h-16 md:h-20 flex items-center justify-between">
                    <div className="flex-1">
                        <LogoIcon size={32} className={`transition-colors duration-300 ${isDarkTheme ? 'text-black' : 'text-white'}`} />
                    </div>
                    <div className="flex-none">
                        <MainLogo width={120} height={40} className={`transition-colors duration-300 ${isDarkTheme ? 'text-black' : 'text-white'}`} />
                    </div>
                    <div className="flex-1 flex justify-end">
                        <button onClick={toggleMenu} className={`p-2 transition-all ${isDarkTheme ? 'text-black' : 'text-white'}`}>
            ...
            >
                <div className="container mx-auto px-6 md:px-12 lg:px-16 pt-16 h-full flex flex-col lg:flex-row justify-between items-center gap-12">
                    ...
                    {/* Image Preview */}
                    <div className="hidden lg:block w-125 h-87.5 relative rounded-2xl overflow-hidden shadow-2xl bg-gray-200">
                        <Image
                            src={DEFAULT_IMAGE}
                            alt="Default View"
                            fill
